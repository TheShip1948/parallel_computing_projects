...
if %errorlevel% neq 0 (
    ...
    nvcc main.cu -o build/MP7.exe -Xcompiler /MD
    
    if not exist build/MP7.exe (
        echo Fallback Build failed!
        exit /b 1
    )
    echo Fallback Build Successful! Executable is in build/MP7.exe
    exit /b 0
)
...
echo Build successful! Executable is: build\MP7.exe
endlocal
