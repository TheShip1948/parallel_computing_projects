...
if %errorlevel% neq 0 (
    ...
    nvcc main.cu -o build/reduction.exe -Xcompiler /MD
    
    if %errorlevel% neq 0 (
        echo Fallback Build failed!
        exit /b %errorlevel%
    )
    echo Fallback Build Successful! Executable is in build/reduction.exe
    exit /b 0
)
...
echo Build successful! Executable is: build\reduction.exe
endlocal
