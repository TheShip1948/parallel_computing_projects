...
mnist_loader::mnist_loader(std::string image_file, std::string label_file) :
...
void mnist_loader::load_images(std::string image_file, int num)
{
  ...
  if (!ifs.is_open()) {
    std::cerr << "Can't open image file: " << image_file << std::endl;
    return;
  }
  char p[4];
...
void mnist_loader::load_labels(std::string label_file, int num)
{
  ...
  if (!ifs.is_open()) {
    std::cerr << "Can't open label file: " << label_file << std::endl;
    return;
  }
  char p[4];
